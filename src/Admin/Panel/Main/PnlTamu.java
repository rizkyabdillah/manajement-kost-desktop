...
public class PnlTamu extends javax.swing.JPanel {

    private final Message msg = new Message();
    private final Koneksi con = new Koneksi();
    private String idTamu = "", namaTamu = "";
    private ResultSet rs;

    private final String header[] = {
        "Id Tamu", "Nama Tamu", "Asal", "No Telp", "Jenis Kelamin"
    };
    ...
    private void showTabel(String key) {
        try {
            String sql = "SELECT * FROM TAMU WHERE "
                    + "id_tamu LIKE '%" + key + "%' OR "
            ...
            tabel.getColumnModel().getColumn(2).setMinWidth(560);
    ...
    private void onTambah(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onTambah
        ...
        new DialogAddTamu(null, true).setVisible(true);
        showTabel("");
        idTamu = "";
    ...
    private void onHapus(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onHapus
        try {
            if(idTamu.equals("")) {
                ...
                int i = msg.msgQuestion("Apakah anda yakin ingin menghapus data " + namaTamu + " ?");
                if(i == JOptionPane.YES_OPTION) {
                    con.setExecute("DELETE FROM TAMU WHERE id_tamu ='" + idTamu + "'");
                }
            }
            this.idTamu = "";
            showTabel("");
    ...
    private void onPilihTabel(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onPilihTabel
        try {
            idTamu = tabel.getValueAt(tabel.getSelectedRow(), 0).toString();
            namaTamu = tabel.getValueAt(tabel.getSelectedRow(), 1).toString();
        } catch (Exception e) {
            idTamu = "";
