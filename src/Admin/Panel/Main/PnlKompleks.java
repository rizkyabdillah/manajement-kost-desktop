...
public class PnlKompleks extends javax.swing.JPanel {
    ...
    private final String header[] = {
        "Id Kompleks", "Nama Ketua RT", "Nama Satpam", "Alamat Kompleks"
    };
    ...
    private void showTabel(String key) {
        try {
            String sql = "SELECT * FROM VIEW_KOMPLEKS WHERE "
                    + "id_kompleks LIKE '%" + key + "%' OR "
            ...
            tabel.getColumnModel().getColumn(3).setMinWidth(660);
        } catch (Exception e) {
    ...
    private void onTambah(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onTambah
        ...
        new Admin.Dialog.Master.DialogAddKompleks(null, true).setVisible(true);
        showTabel("");
        id = "";
    ...
    private void onHapus(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onHapus
        try {
            if(id.equals("")) {
                ...
                int i = msg.msgQuestion("Apakah anda yakin ingin menghapus data " + id + " ?");
                if(i == JOptionPane.YES_OPTION) {
                    con.setExecute("DELETE FROM KOMPLEKS WHERE id_kompleks ='" + id + "'");
                }
            }
            this.id = "";
            showTabel("");
