...
public class DialogShowDetailRiwayat extends javax.swing.JDialog {
    
    private final Koneksi con = new Koneksi();
    private final Message msg = new Message();
    private String idPenghuni = "", idKostt = "", waktu;

    private final String header[] = {
        "ID Penghuni", "ID Kost", "Nama Penghuni", "Nama Kost", "Tanggal Masuk", "Tanggal Keluar"
    };

    public DialogShowDetailRiwayat(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        showTabel("");
    }
    
    public DialogShowDetailRiwayat(java.awt.Frame parent, boolean modal, String idKost) {
        super(parent, modal);
        initComponents();
        this.idKostt = idKost;
        showTabel("");
     ...
     private void showTabel(String key) {
        try {
            String sql = "SELECT * FROM VIEW_LOG_BUKU_PENGHUNI_KOST WHERE "
                + "id_kost ='" + idKostt + "' AND ("
    ...
    private void initComponents() {
        ...
        btnHapus2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images1/a_hapus.png"))); // NOI18N
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/a_riwayat_120.png"))); // NOI18N
        jLabel1.setText(" FORM DETAIL RIWAYAT PENGHUNI");
    ...
    private void onPilihTabel(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onPilihTabel
        try {
            idPenghuni = tabel.getValueAt(tabel.getSelectedRow(), 0).toString();
            waktu = tabel.getValueAt(tabel.getSelectedRow(), 4).toString();
        } catch (Exception e) {
            idPenghuni = "";
    ...
    private void onHapus(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onHapus
        try {
            if(idPenghuni.isEmpty()) {
                ...
                int i = msg.msgQuestion("Apakah anda yakin ingin menghapus log ?");
                if(i == JOptionPane.YES_OPTION) {
                    con.setExecute("DELETE FROM LOG_BUKU_PENGHUNI_KOST "
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DialogShowDetailRiwayat.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        
        java.awt.EventQueue.invokeLater(() -> {
            DialogShowDetailRiwayat dialog = new DialogShowDetailRiwayat(new javax.swing.JFrame(), true);
